"""
Phase 3 - Category B2: Outbreak Prediction Model

Uses aggregated temporal patterns to train a regression model that predicts
...
Steps:
  ...
  3) Train/test split and train a RandomForestRegressor
  4) Compute RMSE and MAE
  5) Store predictions in `outbreak_predictions` table

Run locally (with oc port-forward):
  1) oc port-forward svc/postgres 5432:5432
  2) python features/category_b/outbreak_prediction.py
"""
...
def build_features(df: pd.DataFrame):
    ...
    # Ensure numeric types (defensive casting)
    for col in ["day_of_week", "hour_of_day", "unique_patients", "event_count"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")

    # Drop rows with missing values in key columns
    df = df.dropna(subset=["day_of_week", "hour_of_day", "unique_patients", "event_count"])
    if df.empty:
    ...
    # Simple cyclical encoding for hour_of_day
...
def train_and_evaluate(X, y, test_size: float = 0.2, random_state: int = 42):
    """
    Train a regression model and compute evaluation metrics.
    """
    ...
    print("Outbreak Prediction Metrics:")
    print(f"  RMSE = {rmse:.3f}")
    print(f"  MAE  = {mae:.3f}")

    # Return model and test set for storing predictions
...
def write_predictions(
    ...
    print(f"Wrote {len(df_pred)} predictions to 'outbreak_predictions' (run_id={run_id}).")


# Entry point

def main():
    print("=== Phase 3: Outbreak Prediction Model ===")
    engine = get_engine()
    ...
    model, X_test, y_test, y_pred, rmse, mae = train_and_evaluate(X, y)
    write_predictions(engine, X_test, y_test, y_pred, rmse, mae)

    print("=== Outbreak prediction run complete ===")
